...
MAINFOLDER = r"D:\APPdata_SierraA22"

# Create other paths relative to the main folder
training_file_path = os.path.join(MAINFOLDER, "Training", "Training_SierraA22.xlsx")  # FIXED TRAINING DATA
test_file_path = os.path.join(MAINFOLDER, "24hrData", "Dailydata_SierraA22.xlsx")  # DAILY DATA
excel_file_path = os.path.join(MAINFOLDER, "Breakdownrecords_SierraA22.xlsx")  # Recording excel for BD
folderpath = os.path.join(MAINFOLDER, "TemporaryData")  # Temporary dump files collector
threshold_file_path = os.path.join(MAINFOLDER,"Thresholds_SierraA22.xlsx") #
...
def process_data():
    ...
    assets_list = [
        "Sierra-1 Burnising Machine MDE","Sierra-1 Burnising Machine MNDE","Sierra-1 Burnising Machine Spindle DE",
        "Sierra-1 Burnising Machine Spindle NDE"
]
...
def train_ensemble_model(training_file_path, model_folder_path):
    ...
    def preprocess_data(X, y):
        ...
        # Save the scaler
        joblib.dump(scaler, os.path.join(model_folder_path, "scaler_SieraaA22.pkl"))
    ...
    joblib.dump(ensemble_model, os.path.join(model_folder_path, "ensemble_SieraaA22.pkl"))
...
def predict_ensemble(test_file_path, model_folder_path):
    ...
    def preprocess_test_data(X_test):
        scaler = joblib.load(os.path.join(model_folder_path, "scaler_SieraaA22.pkl"))
        X_test_scaled = scaler.transform(X_test)
        return X_test_scaled

    def predict(X_test_scaled):
        nn_model = joblib.load(os.path.join(model_folder_path, "ensemble_SieraaA22.pkl"))
